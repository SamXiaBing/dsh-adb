...
- **M3**：台架端到端冒烟（SA_DIREWOLF_IVI）→ 抓到 bug ①②，发 0.1.1/0.1.2；补测全部 7 工具 → 抓到 bug ③，发 0.1.3。
- **M4（真机补测）**：用户插入 Redmi K50 Pro → 抓到 bug ④（battery），发 0.1.4；补测关闭全部环境缺口（battery 实数据、正向无线连接）。至此「提交即测」达成。
